...
from aioecopanel import DeviceDict, EcoPanelConnectionError, Interface, EcoPanelEmptyResponseError
...
class EcoPanelConfigFlow(ConfigFlow, domain=DOMAIN):
	...
	async def async_step_user(
		...
		if user_input is not None:
			try:
				devicedict = await self._async_get_device(user_input[CONF_HOST],user_input[CONF_PORT])
			...
			data_schema=vol.Schema(
				{
					...
					vol.Required(
						CONF_PORT, 
						description={
							"suggested_value": self.config_entry.data.get(CONF_PORT, 8099)
							},
					): int,
				}
			),
